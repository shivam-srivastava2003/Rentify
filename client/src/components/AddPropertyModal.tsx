...
const AddPropertyModal: React.FC<AddPropertyModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  // Image Upload Handler with STRICT Validation (Max 4 images, JPG/PNG only, <= 2MB)
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (images.length + files.length > 4) {
      setFlash({
        type: 'error',
        message: 'Maximum limit exceeded! You can upload a maximum of 4 property images only.',
      });
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (images.length === 0) {
      setFlash({ type: 'error', message: 'Please upload at least 1 property photo (Max 4 images).' });
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        setFlash({ type: 'success', message: 'Property posted successfully!' });
        setTimeout(() => {
          onSuccess();
          onClose();
        }, 800);
      ...
      setFlash({
        type: 'error',
        message: err.response?.data?.message || err.message || 'Failed to post property. Please try again.',
      });
    } finally {
      setLoading(false);
    }
      ...
      <div className="bg-white rounded-3xl shadow-2xl max-w-3xl w-full border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="bg-slate-900 text-white p-6 flex items-center justify-between border-b border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-extrabold tracking-tight">Post Property & PG Unit</h2>
              <p className="text-xs text-slate-400">List your rental property on Rentify for direct tenant leads</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          <form id="add-property-form" onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              <h3 className="text-xs font-bold text-teal-800 uppercase tracking-wider flex items-center gap-1.5 pb-2 border-b border-slate-100">
                <Building2 className="w-4 h-4 text-teal-600" /> 1. Basic Property Information
              </h3>
            ...
            {/* SECTION 2: STRICT IMAGE UPLOAD VALIDATION */}
            <div className="space-y-4">
              ...
              <div className="p-4 bg-slate-50 border-2 border-dashed border-slate-300 rounded-2xl text-center">
                ...
                >
                  ...
                  <span className="text-xs font-bold text-slate-900">
                    Click to Upload Images (JPG, PNG only)
                  </span>
                  <span className="text-[11px] text-slate-500 mt-1">
                    Maximum 4 images • Maximum 2 MB per file
                  </span>
                </label>
        ...
        <div className="bg-slate-50 p-4 border-t border-slate-200 flex items-center justify-end gap-3">
          ...
          <button
            ...
            disabled={loading}
            className="bg-teal-600 hover:bg-teal-700 text-white px-6 py-2.5 rounded-xl text-xs font-bold shadow-md shadow-teal-600/20 flex items-center gap-2 transition-all disabled:opacity-50"
          >
            {loading ? (
