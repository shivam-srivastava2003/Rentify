...
const ChatbotWidget: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      ...
      {isOpen && (
        <div className="w-[360px] sm:w-[400px] h-[520px] bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl flex flex-col overflow-hidden mb-4 transition-all duration-300 animate-in fade-in slide-in-from-bottom-5">
          {/* HEADER BAR */}
          <div className="bg-gradient-to-r from-teal-800 via-teal-900 to-slate-950 p-4 text-white flex items-center justify-between border-b border-slate-800/80 shadow-md">
            <div className="flex items-center gap-3">
              <div className="relative">
                <div className="w-10 h-10 rounded-2xl bg-teal-500/20 text-teal-400 border border-teal-500/30 flex items-center justify-center font-bold shadow-xs">
              ...
              <div>
                <h3 className="font-extrabold text-sm flex items-center gap-1.5 leading-tight">
                  <span>Rentify AI Assistant</span>
                  <Sparkles className="w-3.5 h-3.5 text-amber-400 fill-amber-400" />
                </h3>
                <div className="flex items-center gap-2 text-[10px] text-teal-200 mt-0.5 font-medium">
                  <span className="uppercase font-extrabold tracking-wider px-1.5 py-0.2 rounded bg-teal-600/60 text-white">
                    {userRoleString}
                  </span>
                  <span>• Online</span>
                </div>
          ...
          <div className="flex-1 flex flex-col justify-between overflow-hidden bg-slate-950">
            ...
            <div className="px-3 py-2 bg-slate-900/90 border-t border-slate-800/80 flex items-center gap-1.5 overflow-x-auto no-scrollbar">
              {getSuggestionChips().map((chip, idx) => (
                <button
                  ...
                  className="whitespace-nowrap bg-slate-800 hover:bg-teal-600 hover:text-white text-slate-300 text-[10px] font-semibold px-2.5 py-1 rounded-full border border-slate-700 transition-all shrink-0"
                >
                  {chip}
                </button>
              ))}
            ...
            >
              <input
                ...
                className="flex-1 bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-teal-500"
              />
              <button
                type="submit"
                disabled={!inputMessage.trim() || loading}
                className="bg-teal-600 hover:bg-teal-500 disabled:opacity-40 text-white p-2.5 rounded-xl transition-all shadow-md flex items-center justify-center shrink-0"
              >
      ...
      <button
        ...
        className="relative group bg-gradient-to-r from-teal-600 to-teal-800 hover:from-teal-500 hover:to-teal-700 text-white p-4 rounded-2xl shadow-2xl flex items-center justify-center transition-all transform hover:scale-105"
